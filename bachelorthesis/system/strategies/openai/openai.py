########################################################################################################################
# OpenAI API helpers version: 2024-03-28
########################################################################################################################
...
_openai_cache_path = pathlib.Path("openai_cache")  # TODO: set path
_openai_cache_size = 100_000
...
def _openai_compute_approximate_input_len(request: dict) -> int:
    ...
    if "messages" in request.keys():
        ...
        return sum(
            len(encoding.encode(message["content"])) + extra_tokens
...
def _openai_compute_approximate_max_output_len(request: dict) -> int:
    if request["max_tokens"] is not None:
        ...
        left_for_output = max(
            ...
            - _openai_compute_approximate_input_len(request),
        )
        if (
            model_params["max_output_tokens"] is not None
            and model_params["max_output_tokens"] < left_for_output
        ):
...
def _openai_compute_approximate_max_total_len(request: dict) -> int:
    return _openai_compute_approximate_input_len(
        request
    ) + _openai_compute_approximate_max_output_len(request)
...
def _openai_compute_approximate_max_cost(request: dict) -> float:
    ...
    model_params = _openai_get_model_params(request["model"])
    input_cost = _openai_compute_approximate_input_len(request) * (
        model_params["cost_per_1k_input_tokens"] / 1000
    )
    output_cost = _openai_compute_approximate_max_output_len(request) * (
        model_params["cost_per_1k_output_tokens"] / 1000
    )
...
def _openai_check_request(request: dict) -> None:
    ...
    if model_params["max_context"] == approx_input_len:
        logger.warning(
            "Unable to generate any output tokens due to the model's max_context!"
        )

    if request["max_tokens"] is not None:
        if (
            model_params["max_output_tokens"] is not None
            and model_params["max_output_tokens"] < request["max_tokens"]
        ):
            logger.warning(
                "Unable to generate max_tokens output tokens due to the model's max_output_tokens!"
            )

        if model_params["max_context"] < approx_input_len + request["max_tokens"]:
            logger.warning(
                "Unable to generate max_tokens output tokens due to the model's max_context!"
            )

    # if "seed" not in request.keys():
    #     logger.warning("Request does not contain a seed!")

    # if request["temperature"] != 0:
    #     logger.warning("Temperature is not set to 0!")
...
def _openai_load_cached_response(request: dict) -> dict | None:
    ...
    matching_cache_file_paths = list(
        sorted(_openai_cache_path.glob(f"*-{request_hash}.json"))
    )
...
def openai_model(model: str) -> dict:
...
def openai_execute(
    requests: list[dict], *, force: float | None = None, silent: bool = False
) -> list[dict]:
    ...
    total_max_cost = sum(
        _openai_compute_approximate_max_cost(pair["request"])
        for pair in pairs_to_execute
    )
    if force is None or total_max_cost >= force:
        logger.info(
            f"Press enter to continue and spend up to around ${total_max_cost:.2f}."
        )
    ...
    for pair in pairs_to_execute:
        pair["max_total_len"] = _openai_compute_approximate_max_total_len(
            pair["request"]
        )
    pairs_to_execute.sort(key=lambda p: p["max_total_len"], reverse=True)

    # execute requests
    with tqdm.tqdm(
        total=len(pairs), desc="execute requests", disable=silent
    ) as progress_bar:
        for pair in pairs_to_execute:
            ...
            while True:
                ...
                relevant_pairs = [
                    p
                    for p in pairs_to_execute
                    if p["thread"] is not None
                    and not p["was_cached"]
                    and (
                        p["finished_time"] is None
                        or time.time() - p["finished_time"] <= _openai_wait_window
                    )
                ]
                total_requests = len(relevant_pairs)
                simultaneous_requests = len(
                    [p for p in relevant_pairs if p["finished_time"] is None]
                )
    ...
    if len(cache_file_paths) > _openai_cache_size:
        logger.warning(
            f"OpenAI cache is too large ({len(cache_file_paths)} > {_openai_cache_size}) and will be shrunk!"
        )
    ...
    total_cost = sum(
        _openai_compute_actual_cost(pair["response"])
        for pair in pairs_to_execute
        if not pair["was_cached"]
    )
    if not silent:
